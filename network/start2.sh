...
peer lifecycle chaincode approveformyorg -o localhost:7050 \
  --ordererTLSHostnameOverride orderer.wasteManagement.com \
  --channelID $CHANNEL_NAME \
  --name wasteManagementzip \
  --version 1.0 \
  --collections-config ${PWD}/../chaincode/collection-config.json \
...
# peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.wasteManagement.com --channelID $CHANNEL_NAME --name wasteManagementzip --version 1.0 --collections-config ${PWD}/../chaincode/collection-config.json --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile $ORDERER_CA --waitForEvent
sleep 2
...
peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name wasteManagementzip --version 1.0 --sequence 1 --collections-config ${PWD}/../chaincode/collection-config.json --tls --cafile $ORDERER_CA --output json

peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.wasteManagement.com \
--channelID $CHANNEL_NAME --name wasteManagementzip \
--version 1.0 --sequence 1 --collections-config ${PWD}/../chaincode/collection-config.json \
--tls --cafile $ORDERER_CA \
--peerAddresses localhost:5051 --tlsRootCertFiles $wasteCollectionCompany_PEER_TLSROOTCERT \
--peerAddresses localhost:7051 --tlsRootCertFiles $recyclingcenter_PEER_TLSROOTCERT \
--peerAddresses localhost:6051 --tlsRootCertFiles $manufacture_PEER_TLSROOTCERT \
